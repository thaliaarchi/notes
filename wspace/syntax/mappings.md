...
[hohoho](https://github.com/freyamade) uses `snow` for space, `hohoho` for tab,
and LF as itself. When converting from Whitespace to hohoho, spaces are added
after `snow` and `hohoho`, but when parsing, the spaces are ignored. It is
